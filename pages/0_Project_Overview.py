...
st.success(
    "Use the labelled navigation in the sidebar to explore the "
    "analysis. A useful starting point is Sales Overview, followed by "
    "Product Analysis, Market Analysis and Customer Segmentation."
)

st.subheader("Dashboard guide")

guide_column_one, guide_column_two = st.columns(2)

with guide_column_one:
    st.markdown(
        """
        #### 📈 Sales Overview

        Filter completed sales by date and country, examine dynamic KPIs,
        explore revenue trends and review invoice-value distributions.

        #### 📦 Product Analysis

        Rank products by revenue, units sold or invoice reach and distinguish
        broadly popular products from isolated bulk purchases.

        #### 🌍 Market Analysis

        Compare geographic markets using revenue rankings, average invoice
        values and an interactive country map.
        """
    )

with guide_column_two:
    st.markdown(
        """
        #### ↩️ Cancellation Analysis

        Separate monetary cancellations from zero-price adjustments and
        identify products with high cancellation values.

        #### 👥 Customer Segmentation

        Explore the four K-Means RFM segments and download filtered customer
        audiences for targeted marketing.

        #### 🎯 Marketing Campaign Planner

        Test campaign assumptions, estimated contribution, ROI and break-even
